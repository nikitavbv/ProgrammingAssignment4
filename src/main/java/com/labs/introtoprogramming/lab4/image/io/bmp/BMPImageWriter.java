...
public class BMPImageWriter implements ImageWriter {
  ...
  ByteBuffer headerAsByteBuffer(int dibHeaderSize, int pixelStorageSize) {
  ...
  ByteBuffer dibHeaderAsByteBuffer(RGBImage image, int pixelStorageSize) {
  ...
  ByteBuffer pixelStorageAsByteBuffer(RGBImage image) {
    ByteBuffer pixelStorage = ByteBuffer.allocate(getPixelStorageRowSize(image) * image.height());
    pixelStorage.order(ByteOrder.LITTLE_ENDIAN);
    for (int i = image.height() - 1; i >= 0; i--) {
      pixelStorage.put(pixelStorageRowAsByteBuffer(image, i).array());
    }
    return pixelStorage;
  }

  ByteBuffer pixelStorageRowAsByteBuffer(RGBImage image, int row) {
    ByteBuffer rowData = ByteBuffer.allocate(getPixelStorageRowSize(image));
    rowData.order(ByteOrder.LITTLE_ENDIAN);
